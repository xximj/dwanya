  ...
  renderModlogType(i: ModlogType) {
    switch (i.type_) {
      case ModlogEnum.ModRemovePost: {
        ...
        return [
          mrpv.mod_remove_post.removed ? "🚫 حذف " : "↩️ استعاد ",
          <span>
            المنشور <Link to={`/post/${mrpv.post.id}`}>{mrpv.post.name}</Link>
          </span>,
          mrpv.mod_remove_post.reason &&
            ` السبب: ${mrpv.mod_remove_post.reason}`,
        ];
      }
      case ModlogEnum.ModLockPost: {
        let mlpv = i.view as ModLockPostView;
        return [
          mlpv.mod_lock_post.locked ? "📫 أقفل التعليقات " : "📭 فتح التعليقات ",
          <span>
            لـ <Link to={`/post/${mlpv.post.id}`}>{mlpv.post.name}</Link>
          </span>,
        ];
      }
      case ModlogEnum.ModStickyPost: {
        ...
        return [
          mspv.mod_sticky_post.stickied ? "📌 قام بالتثبيت " : "↪️ ألغى التثبيت ",
          <span>
            لـ <Link to={`/post/${mspv.post.id}`}>{mspv.post.name}</Link>
          </span>,
        ];
      }
      case ModlogEnum.ModRemoveComment: {
        ...
        return [
          mrc.mod_remove_comment.removed ? "🚫 حذف " : "↩️ استعاد ",
          <span>
            التعليق {" "}
          ...
          <span>
            {" "}
            بواسطة <PersonListing person={mrc.commenter} />
          </span>,
          mrc.mod_remove_comment.reason &&
            ` السبب: ${mrc.mod_remove_comment.reason}`,
        ];
      }
      case ModlogEnum.ModRemoveCommunity: {
        let mrco = i.view as ModRemoveCommunityView;
        return [
          mrco.mod_remove_community.removed ? "❌ حظر " : "📢 استعاد ",
          <span>
            المجتمع <CommunityLink community={mrco.community} />
          </span>,
          mrco.mod_remove_community.reason &&
            ` السبب: ${mrco.mod_remove_community.reason}`,
      ...
      case ModlogEnum.ModBanFromCommunity: {
        ...
        return [
          <span>
            {mbfc.mod_ban_from_community.banned ? "🗿 حظر " : "🤝 أضاف "}{" "}
          </span>,
          <span>
          ...
          <div>
            {mbfc.mod_ban_from_community.reason &&
              ` السبب: ${mbfc.mod_ban_from_community.reason}`}
          </div>,
          <div>
      ...
      case ModlogEnum.ModAddCommunity: {
        ...
        return [
          <span>
            {mac.mod_add_community.removed ? "❌ غادر " : "🎉 أضاف "}{" "}
          </span>,
          <span>
            <PersonListing person={mac.modded_person} />
          </span>,
          <span>
            {mac.mod_add_community.removed ? " من " : " إلى "}{" "}
          </span>,
          <span> فريق الإشراف في </span>,
          <span>
      ...
      case ModlogEnum.ModTransferCommunity: {
        ...
        return [
          <span>
            {mtc.mod_transfer_community.removed ? "✋ ألغى نقل ملكية " : "📝 نقل ملكية "}{" "}
          </span>,
          <span>
      ...
      case ModlogEnum.ModBan: {
        ...
        return [
          <span>{mb.mod_ban.banned ? "⛔ حظر " : "🙌 فك الحظر عن "} </span>,
          <span>
            <PersonListing person={mb.banned_person} />
          </span>,
          <div>{mb.mod_ban.reason && ` السبب: ${mb.mod_ban.reason}`}</div>,
          <div>
            {mb.mod_ban.expires &&
              ` ⏰ ينتهي ${moment.utc(mb.mod_ban.expires).fromNow()}`}
          </div>,
        ];
      }
      case ModlogEnum.ModAdd: {
        ...
        return [
          <span>{ma.mod_add.removed ? "⭕ غادر " : "🌟 أضاف "} </span>,
          <span>
            <PersonListing person={ma.modded_person} />
          </span>,
          <span>{ma.mod_add.removed ? " من " : " إلى "} </span>,
